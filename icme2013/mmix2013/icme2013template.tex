...
We describe our exact requirements on media fragment identifiers
in \autoref{sec:media-fragment-requirements}.
...
we detail the way the tile-wise histogram-based
media item deduplication and clustering algorithm works.
In \autoref{sec:debugging-the-algorithm},
we show low-level debugging approaches
in order for human raters to understand
why or why not media items are clustered.
In \autoref{sec:from-debug-output-to-story},
we elaborate on how low-level debug output gets lifted
to natural language stories that help human raters grasp
...
\begin{enumerate}
  ...
  \item If the colors of a~tile and its counterpart tile are within the
...
(\emph{e.g.}, the video borders in \autoref{fig:near-duplicate})
...
\begin{figure}[t!]
  ...
  \caption{Debug view of the media item deduplication algorithm
    ...
    the match happens solely based on tile similarity; black tiles are not considered)}
  \label{fig:algorithmdebug}  
\end{figure}

\section{Debugging the Algorithm}
