...
that is to be highlighted first,
which is overall tiles statistics.
...
when statements about tile(s) or face(s) are made.
Exactness and approximation of calculated values needs to be expressed.
Finally, judgements in appositions have to be carefully balanced.

\subsubsection{Realization}

In the following quotes, we show some examples of actual generated texts
for different debug scenarios. 
The running example of the \emph{Backstreet Boys}
music video \emph{Everybody} media items
is the first example below.

\noindent \textbf{Clustering Consent}

\begin{quote}
  \textit{``The two media items are near-duplicates.
...
\begin{quote}
  \textit{``The two media items are exact duplicates.
...
\noindent \textbf{Clustering Dissent}
...
\noindent \textbf{Non-Clustering Dissent}

\begin{quote}
  \textit{``The two media items are different.
...
\noindent \textbf{(Non-Clustering Consent)}
