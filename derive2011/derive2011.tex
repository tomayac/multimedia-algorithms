...
%%%%%%%%% ABSTRACT
\begin{abstract}
Albeit considerable efforts have been put in making video content on the Web more accessible, searchable, and navigable by both textual and visual analysis of the content, most of the time videos still are relatively opaque objects in websites. With the ongoing process of Web browsers gaining more and more support for the HTML5 \texttt{video} tag, at least mark-up-wise videos are becoming first class citizens on the Web. In this paper, we show how through a combination of \emph{textual}, \emph{visual}, and \emph{behavioral} analysis events can be detected on-the-fly in YouTube videos. We therefore define two types of events: visible events in the sense of scene changes, and interest-based events in the sense of purposeful in-video navigation. We realize this by means of a Web browser extension that seamlessly integrates into the YouTube homepage. This extension shows points of interest in the video, contained named entities with their depictions, and the different scenes.
\end{abstract}
