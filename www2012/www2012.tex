...
%\\ and Crowdsourcing In-Video Hot Spot Identification
}
...
\additionalauthors{Arnaud Brousseau (Google Germany GmbH intern, EURECOM M.Sc. student, email: \texttt{arnaud.brousseau@gmail.com}).}
\maketitle
\begin{abstract}
...
%We then explain how the generated set of shots of a video drives crowdsourced identification of ``hot spots'', interesting shots watched by many viewers.
...
%We therefore use Google Analytics event tracking~\footnote{\url{http://code.google.com/apis/analytics/docs/tracking/eventTrackerGuide.html}}.
...
%For the task of suggesting video thumbnails, a combined crowdsourced hot spot-based approach can lead to more accurate results than the current purely computer vision-based approach.
...
\section{Acknowledgments} % Acknowledgments have a Section number in the ACM template
