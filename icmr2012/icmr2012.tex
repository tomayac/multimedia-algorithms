...
For this paper, we consider 11 \todo{make sure final number is correct} social networks and in \autoref{tab:networks} provide a categorization according to the schema defined above.
The list of considered social networks is non-exhaustive.
...
\begin{table*}[htbp]
  \begin{tabular}{ | c | c | c | p{8cm} |}
    ...
    \hline
	Google+ & \url{http://google.com/+} & Second order & Links to media items are returned via the Google+ API.\\\hline
	MySpace & \url{http://myspace.com} & Second order & Links to media items are returned via the MySpace API.\\\hline
	Facebook & \url{http://facebook.com} & Second order & Links to media items are returned via the Facebook API.\\\hline
	Twitter & \url{http://twitter.com} & Second order & Many people use Twitter in third order mode with other media platforms. In second order mode, links to media items are returned via the Twitter API. In third order mode, Web scraping or media platform API usage are necessary to retrieve links to media items.\\\hline
	Instagram & \url{http://instagram.com} & First order & Links to media items are returned via the Instagram API.\\\hline
	YouTube & \url{http://youtube.com} & First order & Links to media items are returned via the YouTube API.\\\hline
	Flickr & \url{http://flickr.com} & First order & Links to media items are returned via the Flickr API.\\\hline
	MobyPicture & \url{http://mobypicture.com} & First order & Popular as media platform for Twitter. Links to media items are returned via the MobyPicture API.\\\hline
	Twitpic & \url{http://twitpic.com} & First order & Popular as media platform for Twitter. Links to media items must be retrieved via Web scraping.\\\hline
	img.ly & \url{http://img.ly} & First order & Popular as media platform for Twitter. Links to media items must be retrieved via Web scraping.\\\hline
	yfrog & \url{http://yfrog.com} & First order & Popular as media platform for Twitter.   Links to media items must be retrieved via Web scraping.\\
	\hline
  \end{tabular}
  \label{tab:networks}
...
Social networks offer a search functionality that allows their content to be queried based on search terms,
