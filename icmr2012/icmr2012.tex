...
Visual clustering, images contained in videos.
}
\end{abstract}
...
\begin{description}
  \item[Google+] (\url{http://google.com/+}) Second order media item support. Links to media items are returned via the Google+ API.
  \item[MySpace] (\url{http://myspace.com}) Second order media item support. Links to media items are returned via the MySpace API.
  \item[Facebook] (\url{http://facebook.com}) Second order media item support. Links to media items are returned via the Facebook API.
  \item[Twitter] (http://twitter.com) Second order media item support, with many people using it in third order mode with other media platforms. In second order mode, links to media items are returned via the Twitter API. In third order mode, Web scraping or media platform API usage are necessary to retrieve links to media items.
  \item[Instagram] (\url{http://instagram.com}) First order media item support. Links to media items are returned via the Instagram API.
  \item[YouTube] (\url{http://youtube.com}) First order media item support. Links to media items are returned via the YouTube API.
  \item[Flickr] (\url{http://flickr.com}) First order media item support. Links to media items are returned via the Flickr API.
  \item[MobyPicture] (\url{http://mobypicture.com}) First order media item support, popular as media platform for Twitter. Links to media items are returned via the MobyPicture API.
  \item[Twitpic] (\url{http://twitpic.com}) First order media item support, popular as media platform for Twitter. Links to media items must be retrieved via Web scraping.
  \item[img.ly] (\url{http://img.ly}) First order media item support, popular as media platform for Twitter. Links to media items must be retrieved via Web scraping.
  \item[yfrog] (\url{http://yfrog.com}) First order media item support, popular as media platform for Twitter.   Links to media items must be retrieved via Web scraping.
\end{description}
...
\todo{
Deduplication of images and videos.
Detection of images contained in videos.
