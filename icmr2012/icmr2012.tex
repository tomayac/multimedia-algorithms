...
\subsection{Dataset}
During the examination of our data set, we observed that the process of image de-duplication is by no means a solved issue.
...
We used a CBIR-based image duplication detection software\footnote{PhotoSweeper: \url{http://itunes.apple.com/us/app/photosweeper/id463362050?mt=12}} that allows for manual algorithm and threshold selection to detect duplicates in the data set.
...
The main problem with the data set is its diversity.
...
In the context of our data set, this approach is counterproductive, though, as a screenshot of a rectangular IAB $728 \times 90$ ``leaderboard'' banner is treated the same as a standard 3.1 megapixels ($2048 \times 1536$) photo, which, in practice, led to incorrect results.
