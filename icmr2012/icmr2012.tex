...
Some of our long-term goals is to detect the popularity of shared media items across social networks.
This task requires the deduplication of extracted media items.
For this paper, we have semi-automatically deduplicated the images with the aid of a CBIR-based image duplication detection software\footnote{PhotoSweeper: \url{http://itunes.apple.com/us/app/photosweeper/id463362050?mt=12}}, and have completely manually deduplicated the videos.
...
The image duplication software we used allows for different algorithms to be used.
...
This method helps find similar photos despite differences in color saturation and lighting.
...
We have deduplicated the videos in the dataset by first splitting them in shots~\cite{CrowdsourcingEvent}, and then manually comparing the videos shot-wise.
We considered \emph{exact duplicates} the videos that shared the same shots, and the same length.
For \emph{loose duplicates}, we entirely manually decided whether the videos showed loosely the same based on human judgment.
We do, at this point, not claim that for loosely similar video detection our results are algorithmically reproducible.
...
\todo{
Deduplication of photos and videos.
